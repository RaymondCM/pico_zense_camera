...
def ip_camera():
    ...
    i2c_publisher = rospy.get_param('~i2c_publisher', "/odroid/weather_board2")
    ...
    # Allow 10 seconds stabilisation (auto-exposure etc)
    rospy.sleep(10)

    rate = rospy.Rate(1.0 / period)

    save_data_folder = 'ip_cam_saved_data/' + datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3] + '/'
    ...
    while not rospy.is_shutdown():
        ...
        try:
            message = rospy.wait_for_message(i2c_publisher, WeatherBoard2, timeout=10)
            weather_file_name = "{}_weather.pkl".format(unique_file_name)
            with open(weather_file_name, 'wb') as fh:
                pickle.dump(message, fh)
            save_message += "\n\t" + weather_file_name
        except rospy.ROSException:
            rospy.logerr("Could not get weather data from '{}'. Skipping.".format(i2c_publisher))
            rate.sleep()
            continue
